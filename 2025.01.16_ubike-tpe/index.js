...
function getTravelRecommendation(e) {
  ...
  const offcanvasExampleEl = document.querySelector(
    "#travel-recommendation-info"
  );

  const myHeaders = new Headers();
  myHeaders.append("Content-Type", "application/json");
  myHeaders.append("Authorization", "Bearer app-UXa1PAGPvKRnnxBCaCVOzC23");

  const raw = JSON.stringify({
    inputs: {
      location: `${stationLocationEL.innerText}`,
      station: `${stationNameEL.innerText}`,
    },
    user: "heric100@hotmail.com",
  ...
  fetch("https://agent.build-school.com/v1/workflows/run", requestOptions)
    ...
    .then((result) => (offcanvasExampleEl.innerText = result.data.outputs.text))
    .catch((error) => console.error(error));
  // console.log(
  //   stationNameEL,
  //   stationLocationEL,
  //   stationNameEL.innerText.split("：")[1],
  //   stationLocationEL.innerText.split("：")[1]
  // );

  // console.log(offcanvasExampleEl.innerText);
